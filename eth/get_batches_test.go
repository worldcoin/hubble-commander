...
func (s *GetBatchesTestSuite) TestGetBatches() {
	...
	batch1, _, err := s.client.SubmitTransfersBatchAndWait([]models.Commitment{commitment1})
	s.NoError(err)
	_, _, err = s.client.SubmitTransfersBatchAndWait([]models.Commitment{commitment2})
	s.NoError(err)
