...
func (s *StateTree) LeafNode(index uint32) (*models.StateNode, error) {
	...
	return s.storage.GetStateNodeByStateID(leafPath)
}
...
func (s *StateTree) unsafeSet(index uint32, state *models.UserState) (err error) {
	...
	currentRoot, err := s.updateStateNodes(&prevLeaf.StateID, &currentLeaf.DataHash)
	if err != nil {
		return
	}
...
func (s *StateTree) calculateParentHash(
	...
	witness, err := s.storage.GetStateNodeByStateID(&witnessPath)
	if err != nil {
