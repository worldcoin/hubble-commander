...
func TestTransaction_UnmarshalJSON_Create2Transfer(t *testing.T) {
	// nolint:lll
	input := `{"Type":3,"FromStateID":1,"ToStateID":2,"ToPublicKey":"","Amount":"50","Fee":"10","Nonce":"0","Signature":"0xdeadbeef"}` // nolint:goconst,lll
	var tx Transaction
	err := json.Unmarshal([]byte(input), &tx)
	require.NoError(t, err)
	require.NotNil(t, tx.Parsed)
