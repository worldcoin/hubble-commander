...
func NewCommander(cfg *config.Config, blockchain chain.Connection) *Commander {
	return &Commander{
		...
		requestsChan:   make(chan *eth.TxSendingRequest, 1024),
	}
}
...
func (c *Commander) Start() (err error) {
	...
	c.client, err = getClient(c.blockchain, c.storage, c.cfg, c.metrics, c.requestsChan)
	if err != nil {
		return err
	}

	c.txsTracker, err = tracker.NewTracker(c.client, c.requestsChan)
	if err != nil {
		return err
	}
	...
	c.startWorker("Tracking Txs", func() error { return c.txsTracker.TrackTxs(c.workersContext) })
...
func getClient(
	...
	if dbChainState != nil {
		...
		return createClientFromChainState(blockchain, dbChainState, cfg, commanderMetrics, requestsChan)
	}

	if cfg.Bootstrap.ChainSpecPath != nil {
		return bootstrapFromChainState(blockchain, storage, cfg, commanderMetrics, requestsChan)
	}
	if cfg.Bootstrap.BootstrapNodeURL != nil {
		log.Printf("Bootstrapping genesis state from node %s", *cfg.Bootstrap.BootstrapNodeURL)
		return bootstrapFromRemoteState(blockchain, storage, cfg, commanderMetrics, requestsChan)
	}
...
func bootstrapFromChainState(
	...
	return bootstrapChainStateAndCommander(blockchain, storage, importedChainState, cfg, commanderMetrics, requestsChan)
}
...
func bootstrapChainStateAndCommander(
	...
	return setGenesisStateAndCreateClient(blockchain, storage, importedChainState, cfg, commanderMetrics, requestsChan)
}
...
func bootstrapFromRemoteState(
	...
	return setGenesisStateAndCreateClient(blockchain, storage, fetchedChainState, cfg, commanderMetrics, requestsChan)
}
...
func setGenesisStateAndCreateClient(
	...
	client, err := createClientFromChainState(blockchain, chainState, cfg, commanderMetrics, requestsChan)
	if err != nil {
		return nil, err
	}
