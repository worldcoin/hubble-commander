...
func (s *SubmitC2TBatchTestSuite) SetupTest() {
	s.testSuiteWithTransactionsContext.SetupTest(batchtype.Create2Transfer)
	s.setupUser()
}
...
func (s *SubmitC2TBatchTestSuite) TestSubmitBatch_SubmitsCommitmentsOnChain() {
	...
	pendingBatch, err := s.transactionsCtx.NewPendingBatch(batchtype.Create2Transfer)
	s.NoError(err)
	err = s.transactionsCtx.SubmitBatch(pendingBatch, []models.CommitmentWithTxs{commitment})
	s.NoError(err)
