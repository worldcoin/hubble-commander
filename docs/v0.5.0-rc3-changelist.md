...
- Data returned from API was revamped so that it's more informative and consistent throughout the endpoints.
- Added more API endpoints and removed single unused one.
- Added commander metrics.
- Added support for deposits and withdrawals (mass migrations).

# API Changes `v0.5.0-rc2` -> `v0.5.0-rc3`

- Added and depricated some API errors. Referer to the API error table in `api/Readme.md` for more detailed information.
- Added multiple proof endpoints. Refer to `docs/json_rpc.md` file for more detailed information.
- Introduced `admin` namespace for the new commander management API endpoints.
- Important data model changes:
    - `Batch`
        - New field:
            - `Status` - string (status):
                - `"SUBMITTED"`
                - `"MINED"` (previously known as `"IN_BATCH"`)
                - `"FINALIZED"`
        - Other:
    ...
    - `Commitment`
        - Added new status. Currently available statuses for commitments:
            - `"SUBMITTED"`
            - `"MINED"` (previously known as `"IN_BATCH"`)
            - `"FINALIZED"`
...
- API Changes:
    - Removed `hubble_getTransactions` endpoint.
    - Added a few endpoints for generating proofs. See `docs/json_rpc.md` for more details.
    - All endpoints now support every type of batch, commitment or transaction, i.e.:
        - `GENESIS`
        - `TRANSFER`
        - `CREATE2TRANSFER`
        - `MASS_MIGRATION`
